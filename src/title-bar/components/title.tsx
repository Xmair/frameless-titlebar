...
const getMargin = (
  ...
  if (align === "center") {
    return margin === "left" && hasIcon && (!hasMenu || style === "vertical")
      ? "0px"
      : "auto";
  }
...
const Title = ({
  ...
  const {
    bar: { height, title, inActiveOpacity },
    menu: { style },
  } = useContext(ThemeContext);
  const { color, align, fontFamily, fontWeight } =
    title as Required<TitleTheme>;
  const marginLeft = ignoreMargin
    ? "0px"
    : getMargin("left", align, hasIcon, hasMenu, style!);
  const marginRight = ignoreMargin
    ? "0px"
    : getMargin("right", align, hasIcon, hasMenu, style!);
